...
<div style="width: 1200px;margin-top: 20px">
    @if(count($errors)>0)
        ...
        @endforeach
    @endif
    <form class="layui-form" action="{{secure_url('admin/teacher')}}" method="post">
        <input type="hidden" name="_token" class="tag_token" value="{{csrf_token()}}">
        <div class="layui-form-item">
            <label class="layui-form-label">教师姓名</label>
            <div class="layui-input-block">
                <input type="text" name="teacher_name"  lay-verify="required" placeholder="请输入" autocomplete="off" class="layui-input">
            </div>
        </div>
        <div class="layui-form-item">
            <label class="layui-form-label">教师性别</label>
            <div class="layui-input-block">
                <input type="radio" name="teacher_gender" value="男" title="男" checked="">
                <input type="radio" name="teacher_gender" value="女" title="女">
            </div>
        </div>
        <div class="layui-form-item">
            <label class="layui-form-label">教师年龄</label>
            <div class="layui-input-block">
                <input type="text" name="teacher_age"   lay-verify="required|number" placeholder="请输入" autocomplete="off" class="layui-input">
            </div>
        </div>
        <div class="layui-form-item">
            <label class="layui-form-label">教师简介</label>
            <div class="layui-input-block">
                <textarea name="teacher_description"   lay-verify="required" placeholder="请输入内容" class="layui-textarea"></textarea>
            </div>
        </div>
        <div class="layui-form-item">
            <label class="layui-form-label">教师照片</label>
            <div class="layui-input-block">
                <input id="teacher_thumb" type="text" name="teacher_thumb" placeholder="上传后自动填写，支持jpg gif png等图片格式" value="" class="layui-input">
            </div>
